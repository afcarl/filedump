...
class BigDump:
    ...
    def __init__(self, path, n_per_file=None, meta_file="metadata"):    
        ...
        if meta_exists:
            if not n_per_file is None:
                raise Exception("metafile found, cannot set n_per_file to {0}".format(n_per_file))
            self._load_metadata()
        elif not meta_exists:
            if n_per_file is None:
                raise Exception("no metafile found, cannot set n_per_file to None")
    ...
    def _select_datafile(self):
        ...
        if self._data_file_handle is None or self._current_data_file != curr_data_file:
            
            # close old handle (if exists)
            if not self._data_file_handle is None:
    ...
    def read(self, begin, end):
        ...
        if end > self._n_stored:
            raise Exception("out of bounds {0} > {1}".format(end, self._n_stored))
